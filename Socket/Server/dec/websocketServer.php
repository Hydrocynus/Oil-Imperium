...
abstract class WebSocketServer {
  ...
  protected $userClass = 'WebSocketUser'; // eigene User Klasse kann verwendet werden, sollte von WebSocketUser erben
  ...
  function __construct($addr, $port, $bufferLength = 2048) {
    ...
    writeLogFile($this->logdat, "-_________________________-sdasdas");
    writeLogFile($this->logdat, "Server started\nListening on: $addr:$port\nMaster socket: ".$this->master);
  }
  
   abstract protected function serverOnMessage($user, $message); // OnMessage: Wird aufgerufen wenn Daten kommen
   abstract protected function serverOnConnect($user);         // OnConnect: Wird nach dem Serverseitigem Handshake aufgerufen
   abstract protected function serverOnClose($user);            // OnClose:  Wird aufgerufen bei Verbindungsabbruch 
  
  
  /**
   * Sendet Nachricht an Client.
   * Speichert Nachricht in $holdingMessage, falls Client momentan nicht verbunden ist. //? bzw kein Handshake da is 
   * @author Tim
   * @version 03.12.2020 
   * @since XX.12.2020
   * @param object $user Socket Objekt, an welches versendet werden soll. 
  ...
  /**
   * //? fusion mit _tick
   * Funktion welche bei jeder run iteration aufgerufen wird.
   * @author Tim
   * @version 03.12.2020 
   * @since XX.12.2020
   */
  ...
  /**
   * //? durchgehen
   * Hauptfunktion, muss aufgerufen werden damit der Server läuft. 
   * @author Tim
   * @version 03.12.2020 
   * @since XX.12.2020
   */
    ...
    while(true) {
      ...
      foreach ($read as $socket) {
        if ($socket == $this->master) {
          ...
          echo $client;
          if ($client < 0) {
            writeLogFile($this->logdat, "Failed: socket_accept()", true);
            continue;
          } 
          else {
            $this->connect($client);
            writeLogFile($this->logdat, "Client connected. " . $client);
          }
        } 
        else {
          ...
          if ($numBytes === false) {
            ...
            switch ($sockErrNo)
            {
              ...
              case 125: // ECANCELED    -- Operation canceled
                ...
                writeLogFile($this->logdat, ("Unusual disconnect on socket " . $socket), true);
              ...
              default:
                ...
                writeLogFile($this->logdat, ('Socket error: ' . socket_strerror($sockErrNo)));
            }
            
          }
          elseif ($numBytes == 0) {
            $this->disconnect($socket);
            writeLogFile($this->logdat, "Client disconnected. TCP connection lost: " . $socket);
          } 
          else {
   ...
   * @version 03.12.2020 
   * @since XX.12.2020
   * @param object Socket Object das sich mit den Server Verbindet. 
   */
  protected function connect($socket) {
    $user = new $this->userClass(uniqid('u'), $socket);
    $this->users[$user->id] = $user;
    $this->sockets[$user->id] = $socket;
    writeLogFile($this->logdat, "Verbindungsversuch von User: " . $user->id);
  }
  
  /**
   * //? selber schreiben
   * Wird beim Disconnecten eines Cleints aufgerufen.
   * Entfernt User vom Server.
   * @author Tim
   * @version 03.12.2020 
   * @since XX.12.2020
   * @param object Socket Object, welches disconnected. 
   * @param boolean (Optional) gibt an ob disconnected werden soll. 
   * @param object (Optional) Socket Fehler Object ?
   */
  protected function disconnect($socket, $triggerClosed = true, $sockErrNo = null) {
    ...
    if ($disconnectedUser !== null) {
      ...
      if ($triggerClosed) {
        writeLogFile($this->logdat, "Client disconnected. ".$disconnectedUser->socket);
        $this->serverOnClose($disconnectedUser);
  ...
  /**
   * //? selber schreiben
   * Serverseitiger Handshake an Client.
   * Wird aufgerufen wenn ein Client Handshake anfrage schickt.
   * Server Verarbeitet Clientseitigen Handshake Header und sendet entsprechenden Serverseitigen Handshake, falls dieser noch nicht gesendet worden ist .
   * @author Tim
   * @version 03.12.2020 
   * @since XX.12.2020
   * @param object User Object.
   * @param int $buffer grösse. 
   */
  protected function doHandshake($user, $buffer) {
    ...
    socket_write($user->socket,$handshakeResponse,strlen($handshakeResponse));
    $this->serverOnConnect($user);
  }
  

  //-- Funktionen um Client Handshake zu prüfen 
  ...
  /**
   * Findet User vom zugehörigen socket. 
   * @author Tim
   * @version 03.12.2020 
   * @since XX.12.2020
  ...
  protected function split_packet($length,$packet, $user) {
    ...
    if (false){ // $user->handlingPartialPacket) { //!
      ...
      if (($message = $this->deframe($frame, $user,$headers)) !== FALSE) {
        if ($user->hasSentClose) {
          ...
          if ((preg_match('//u', $message)) || ($headers['opcode']==2)) {
            ...
            $this->serverOnMessage($user, $message);
          } else {
            writeLogFile($this->logdat, "not UTF-8\n", true);
          }
        }
      } 
  ...
  //Hilfsfunktion: deframe, split_packet ...notwendig...datenmengen? 
